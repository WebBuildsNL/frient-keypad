...
class IasAceBoundCluster extends BoundCluster {
  ...
  constructor({ onArm, onEmergency, onFire, onPanic }) {
    super();
    this._onArm = onArm;
  ...
  async handleFrame(frame, meta, rawFrame) {
    // Ignore global (non-cluster-specific) frames
  ...
  arm(payload) {
    ...
    if (this._onArm) {
      this._onArm({
        ...
        action,
        code: payload.armDisarmCode || '',
        zoneId: payload.zoneId || 0,
      });
    }

    return { armNotification: ACTION_TO_ARM_NOTIFICATION[action] || 'allZonesDisarmed' };
  }
