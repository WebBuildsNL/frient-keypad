...
class IasAceBoundCluster extends BoundCluster {
  ...
  constructor({ endpoint, onArm, onEmergency, onFire, onPanic, validateCode, writeLog }) {
    super();
    ...
    this._validateCode = validateCode || null;
  ...
  arm(payload) {
    ...
    // Arming (lock/home/night) without a code is always allowed.
    // Only disarming (unlock) requires a valid PIN.
    const isArm = action !== 'disarm';
    const validation = (isArm && !code)
      ? { valid: true, name: '', status: 'valid' }
      : (this._validateCode ? this._validateCode(code) : { valid: true, name: '', status: 'valid' });
    ...
    if (this._onArm) {
      this._onArm({
        ...
        codeName: validation.name || '',
        codeStatus: validation.status || 'unknown',
      }).catch((err) => { this._writeLog(`ERROR: onArm callback failed: ${err.message}`); });
    }

    // Respond immediately so the keypad gets LED feedback before its radio sleeps
    // Green LED for valid codes, red LED for invalid codes
    if (validation.valid) {
      const notification = ACTION_TO_ARM_NOTIFICATION[action] || 'allZonesDisarmed';
      return { armNotification: notification };
    }
    return { armNotification: 'invalidCode' };
  }
